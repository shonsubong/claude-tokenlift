...
> **서빙 엔진 선택**: NVIDIA GPU(H200/Blackwell) + 멀티유저라면 **vLLM + NVIDIA/Z.ai 공식
> 양자화(NVFP4/FP8)가 우선 경로**다(연속 배칭 동시성 우수). 이 문서(llama.cpp/GGUF)는
> 저VRAM·CPU offload·혼합 환경의 대안이다. HW별 선택·멀티유저·팀 셋업은
> **[16. GLM-5.2 온프렘 셋업(NVIDIA 공식 우선)](./16-glm-multiquant-team.md)** 참조.
